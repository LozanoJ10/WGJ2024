...
public class MovimientoObjetoTiempo : MonoBehaviour
{
    ...
    public float velocidad = 1f; // Velocidad m�xima de movimiento
    public float tiempoModificador = 0f; // Controla la direcci�n del movimiento, de -1 a 1

    private Vector3 objetivoActual; // Posici�n del objetivo actual
    ...
    void Start()
    {
        ...
        objetivoActual = punto2.position; // Comenzamos movi�ndonos hacia punto2
    ...
    private void OnMouseDown()
    {
        ...
        // Verificar si el clic pas� a trav�s del objeto transparente
        ...
        foreach (RaycastHit hit in hits)
        {
            ...
            {
                Debug.Log("Objeto clickeado detr�s del transparente: " + hit.collider.name);

                // Aqu� puedes manejar el clic en el objeto detr�s del transparente
                MovimientoObjetoTiempo clickedObject = hit.collider.GetComponent<MovimientoObjetoTiempo>();
                if (clickedObject != null)
                {
                    clickedObject.AsignarSliderAlObjeto(); // Asignar el slider al objeto que est� detr�s
                }

                return; // Salir del m�todo una vez que se haya encontrado y manejado el clic en el objeto detr�s
            }
        }
